    ...
    <>
      ...
      <div className="detail-mobile-carousel">
        <div className="mobile-carousel-toolbar">
          ...
          <div className="carousel-controls" aria-label="Kontrol detail layanan">
            <button type="button" onClick={() => scrollByItem(-1)} aria-label="Detail sebelumnya">
              <LineGlyph kind="navLeft" />
            </button>
            <button type="button" onClick={() => scrollByItem(1)} aria-label="Detail berikutnya">
              <LineGlyph kind="navRight" />
            </button>
          </div>
        </div>

        <div className="detail-mobile-viewport native-horizontal-carousel" ref={viewportRef}>
          <div className="detail-mobile-track">
            {panels.map((panel) => (
              <div className="detail-mobile-item" data-carousel-item key={panel.key}>
                {panel.content}
              </div>
            ))}
          </div>
        ...
        <div className="carousel-pagination detail-mobile-pagination" aria-label="Posisi detail layanan">
          ...
          <span className="carousel-count">{String(activeIndex + 1).padStart(2, '0')} / 04</span>
        </div>
      </div>
    </>
  );
